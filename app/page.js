...
export default function SistemaCRAS() {
  ...
  const [mensagemSucesso, setMensagemSucesso] = useState(false); // Novo estado para aviso suave
  ...
  async function salvarCadastro(e) {
    ...
    if (!error) {
      ...
      setMensagemSucesso(true); // Ativa o aviso visual em vez do pop-up
      setTimeout(() => setMensagemSucesso(false), 3000); // Some após 3 segundos
      realizarBusca();
    }
    setLoading(false);
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-100 p-4 font-sans">
        <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-md border-t-8 border-blue-700">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-blue-900 uppercase italic">Portal do Gestor</h1>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      {/* Aviso Sucesso Suave (substitui o pop-up) */}
      {mensagemSucesso && (
        <div className="fixed top-10 right-10 bg-green-600 text-white px-6 py-3 rounded-xl shadow-2xl z-[100] animate-bounce font-bold flex items-center gap-2">
           <Save size={20} /> Cadastro Realizado com Sucesso!
        </div>
      )}

      <header className="bg-blue-700 text-white p-6 shadow-lg border-b-4 border-yellow-500 font-serif italic">
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg p-8 shadow-2xl animate-in zoom-in duration-300">
            ...
            <form onSubmit={salvarCadastro} className="space-y-4">
              ...
              <button disabled={loading} className="w-full bg-blue-700 text-white p-4 rounded-xl font-bold flex items-center justify-center gap-3 hover:bg-blue-800 transition-all shadow-xl shadow-blue-100">
                {loading ? <Loader2 className="animate-spin" /> : "Confirmar Registro Técnico"}
              </button>
            </form>
          </div>
        </div>
